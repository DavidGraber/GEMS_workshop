...
"""
This script evaluates a trained model or an ensemble of models on a test dataset. It loads previously trained models,
runs inference on the provided test dataset, and outputs predictions along with performance metrics.

Description:
    1. Loads one or more trained models specified by state dictionary paths.
    2. Evaluates the model(s) on the provided test dataset.
    3. Calculates and reports performance metrics (RMSE, Pearson correlation, R2 score).
    4. Generates a scatter plot comparing true vs. predicted values.
    5. Saves predictions to a JSON file for further analysis.

REQUIRED Command-line Arguments:
    --stdicts:           REQUIRED - Comma-separated paths to model state dictionaries for ensemble testing
    --dataset_path:      REQUIRED - Path to the test dataset .pt file

OPTIONAL Command-line Arguments:
    --model_arch:        OPTIONAL - Name of the model architecture [GEMS18d, ...]. Default: GEMS18d
    --save_path:         OPTIONAL - Path where results should be saved. Default: Location of test.py

EXAMPLE USAGE:
python test.py --stdicts first_test_f0/first_test_f0_best_stdict.pt --dataset_path datasets/dataset_casf2016.pt
"""
...
def main():
    """
    Main function that handles the evaluation of trained models on a test dataset.
    
    This function:
    1. Parses command-line arguments
    2. Loads the test dataset
    3. Initializes model(s) and loads saved weights
    4. Evaluates the model(s) on the test dataset
    5. Saves predictions and generates visualization plots
    """
    args = parse_args()
